...
const schema = z.object({
  NODE_ENV: z.enum(['development', 'test', 'production']).default('production'), HOST: z.string().default('0.0.0.0'), PORT: z.coerce.number().int().min(1).max(65535).default(8080), LOG_LEVEL: z.enum(['fatal','error','warn','info','debug','trace','silent']).default('info'),
  MAX_UPLOAD_BYTES: z.coerce.number().int().min(1_000_000).max(5_000_000_000).default(524_288_000), UPLOAD_DIR: z.string().min(1).default('.storage/uploads'), DATABASE_URL: z.string().url().optional(), API_KEY: z.string().min(32).optional(), RATE_LIMIT_MAX: z.coerce.number().int().positive().default(60), RATE_LIMIT_WINDOW_MS: z.coerce.number().int().positive().default(60_000), REQUEST_TIMEOUT_MS: z.coerce.number().int().min(5_000).max(900_000).default(120_000), REQUIRE_API_KEY: bool.default(true), TRUST_PROXY: bool.default(false), DELETE_SOURCE_AFTER_VERIFICATION: bool.default(true), PERSISTENT_STORAGE_CONFIRMED: bool.default(false),
  MALWARE_SCAN_URL: z.string().url().optional(), MALWARE_SCAN_TOKEN: z.string().min(16).optional(), MALWARE_SCAN_ALLOW_PRIVATE_HTTP: bool.default(false), MALWARE_SCAN_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(120_000).default(30_000), C2PA_VERIFY_TRUST: bool.default(true), C2PA_OCSP_FETCH: bool.default(false)
});
