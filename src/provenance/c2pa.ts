...
export async function inspectC2pa(path: string, verifyTrust: boolean): Promise<ProvenanceResult> {
  try {
    ...
    return { status: embedded ? (trusted ? 'verified' : 'present_untrusted') : 'absent', embedded, trusted, activeManifest: active ?? undefined, manifestStore: store, errors: hasValidationErrors ? ['C2PA validation reported one or more validation-status entries'] : [] };
  } catch (error) {
