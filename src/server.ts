...
app.addHook('onRequest', async (req, reply) => {
  if (req.url === '/health' || req.url === '/ready' || req.url.startsWith('/public/')) return;
...
app.get('/ready', async (_req, reply) => {
  if (process.env.NODE_ENV === 'production' && !process.env.DATABASE_URL) {
    return reply.code(503).send({ status: 'not_ready', reason: 'DATABASE_URL_REQUIRED' });
  }
  return { status: 'ready' };
});
...
app.post('/v1/media/verify', async (req, reply) => {
  ...
  const record = await verifyMedia(asset, upload.path, {
    verifyTrust: config.C2PA_VERIFY_TRUST,
    requireVerification: true,
  });
  await store.save(record);
  return reply.code(201).send({
    passportId: nanoid(16),
    ...record,
    publicUrl: `/public/${asset.id}`,
  });
});
...
app.get('/public/:id', async (req, reply) => {
  ...
  return {
    passportId: record.asset.id,
    asset: { sha256: record.asset.sha256, mime: record.asset.mime, kind: record.asset.kind, sizeBytes: record.asset.sizeBytes },
    verdict: record.verdict,
    distribution: record.distribution,
    provenance: { status: record.provenance.status, embedded: record.provenance.embedded, trusted: record.provenance.trusted },
...
app.setErrorHandler((error, req, reply) => {
  ...
  if ((error as { code?: string }).code === 'FST_REQ_FILE_TOO_LARGE' || error.message === 'UPLOAD_TOO_LARGE') {
