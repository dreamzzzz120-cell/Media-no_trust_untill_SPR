...
app.addHook('onRequest', async (req, reply) => {
  ...
  if (expected) {
    ...
    if (suppliedBuffer.length === expectedBuffer.length && timingSafeEqual(suppliedBuffer, expectedBuffer)) {
      (req as typeof req & { mediaAuth: ApiIdentity }).mediaAuth = { keyId: 'bootstrap', organizationId: null, role: 'super_admin' };
      return;
    }
  }
  const identity = await store.authenticateApiKey(supplied);
  if (!identity) return reply.code(401).send({ error: 'UNAUTHORIZED' });
  (req as typeof req & { mediaAuth: ApiIdentity }).mediaAuth = identity;
});

function auth(req: typeof app extends never ? never : any): ApiIdentity { return req.mediaAuth as ApiIdentity; }
function requireRole(req: any, reply: any, allowed: ApiRole[]): boolean {
  const identity = auth(req);
  if (!identity || !allowed.includes(identity.role)) { void reply.code(403).send({ error: 'FORBIDDEN' }); return false; }
  return true;
}
...
app.get('/ready', async (_req, reply) => {
  ...
  return { status: 'ready', database: { ok: true }, trustEngine: { ok: true }, security: { uploadQuarantine: true, malwareScanRequired: config.NODE_ENV === 'production' } };
});

app.post('/v1/api-keys', async (req, reply) => {
  if (!requireRole(req, reply, ['super_admin','platform_admin','organization_admin'])) return;
  const identity = auth(req);
  const body = z.object({ organizationId: z.string().min(1), name: z.string().min(1).max(100), role: z.enum(roles as [ApiRole, ...ApiRole[]]) }).parse(req.body);
  if (identity.organizationId && identity.organizationId !== body.organizationId && identity.role !== 'super_admin' && identity.role !== 'platform_admin') return reply.code(403).send({ error: 'TENANT_MISMATCH' });
  const created = await store.createApiKey(body.organizationId, body.name, body.role);
  return reply.code(201).send({ ...created, warning: 'The secret is returned once. Store it securely.' });
});
...
app.post('/v1/media/verify', async (req, reply) => {
  ...
  const identity = auth(req);
  const declaredHeader = String(req.headers['x-declared-ai-use'] ?? 'UNKNOWN').toUpperCase();
  const declaredAiUse = (aiStatuses as readonly string[]).includes(declaredHeader) ? declaredHeader as typeof aiStatuses[number] : 'UNKNOWN';
  const creatorId = typeof req.headers['x-creator-id'] === 'string' && idPattern.test(req.headers['x-creator-id']) ? req.headers['x-creator-id'] : undefined;
  ...
  try {
    ...
    const asset = { id: upload.id, sha256: upload.sha256, mime: upload.mime, kind: upload.kind, sizeBytes: upload.sizeBytes, originalFilename: upload.originalFilename, createdAt: new Date().toISOString(), organizationId: identity.organizationId ?? undefined, creatorId, declaredAiUse } as const;
...
async function getRecord(id: string, reply: any) {
  if (!idPattern.test(id)) { void reply.code(400).send({ error: 'INVALID_ID' }); return null; }
  const record = await store.get(id);
  if (!record) { void reply.code(404).send({ error: 'NOT_FOUND' }); return null; }
  return record;
}

app.get('/v1/media/:id', async (req, reply) => {
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  return record;
});
app.get('/v1/media/:id/evidence', async (req, reply) => {
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  return { passportId: record.asset.id, evidence: record.observations, evidenceQuality: record.trustVector.evidenceQuality };
});
app.get('/v1/media/:id/provenance', async (req, reply) => {
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  return { passportId: record.asset.id, provenance: record.provenance };
});
app.get('/v1/media/:id/trust', async (req, reply) => {
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  return { passportId: record.asset.id, trustScore: record.trustScore, confidence: record.confidence, vector: record.trustVector, decision: record.decision, limitations: record.limitations };
});
app.get('/v1/media/:id/claims', async (req, reply) => {
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  return { passportId: record.asset.id, claims: [], status: 'NOT_ANALYZED', note: 'Claim extraction requires a configured source-analysis provider; no claims are fabricated.' };
});
app.post('/v1/media/:id/appeal', async (req, reply) => {
  if (!requireRole(req, reply, ['creator','organization_admin','platform_admin','super_admin'])) return;
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  const body = z.object({ reason: z.string().min(10).max(5000), evidence: z.array(z.string().max(2000)).max(20).default([]) }).parse(req.body);
  return reply.code(202).send({ status: 'accepted', passportId: record.asset.id, caseId: `APL-${createHash('sha256').update(record.asset.id + body.reason + Date.now()).digest('hex').slice(0, 16)}`, nextStep: 'human_review' });
});
app.post('/v1/media/:id/report', async (req, reply) => {
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  const body = z.object({ category: z.enum(['impersonation','copyright','privacy','deception','spam','other']), description: z.string().min(10).max(5000) }).parse(req.body);
  return reply.code(202).send({ status: 'accepted', passportId: record.asset.id, caseId: `RPT-${createHash('sha256').update(record.asset.id + body.category + Date.now()).digest('hex').slice(0, 16)}` });
});
app.post('/v1/recommendation/evaluate', async (req, reply) => {
  if (!requireRole(req, reply, ['analyst','moderator','organization_admin','platform_admin','super_admin'])) return;
  const body = z.object({ passportId: z.string().min(10).max(40), minTrust: z.number().min(0).max(100).default(70), requireProvenance: z.boolean().default(false), requireDisclosure: z.boolean().default(true) }).parse(req.body);
  const record = await getRecord(body.passportId, reply); if (!record) return;
  const eligible = (record.trustScore ?? 0) >= body.minTrust && (!body.requireProvenance || record.provenance.status === 'verified') && (!body.requireDisclosure || !record.limitations.some(x => /disclosure/i.test(x)));
  return { passportId: record.asset.id, eligible, decision: record.decision, trustScore: record.trustScore, confidence: record.confidence, reasons: eligible ? ['Trust threshold met', 'No blocking decision'] : ['Recommendation criteria not satisfied'], explainability: { trust: record.trustVector, provenance: record.provenance.status, aiStatus: record.aiStatus } };
});

app.get('/public/:id', async (req, reply) => {
  const record = await getRecord((req.params as { id: string }).id, reply); if (!record) return;
  return { passportId: record.asset.id, asset: { sha256: record.asset.sha256, mime: record.asset.mime, kind: record.asset.kind, sizeBytes: record.asset.sizeBytes }, decision: record.decision, trustScore: record.trustScore, confidence: record.confidence, aiStatus: record.aiStatus, provenance: { status: record.provenance.status, embedded: record.provenance.embedded, trusted: record.provenance.trusted }, evidence: record.observations, limitations: record.limitations };
});
...
app.get('/passport/:id', async (req, reply) => {
  ...
  const vector = Object.entries(record.trustVector).map(([k,v]) => `<li><strong>${esc(k)}</strong>: ${esc(v)}</li>`).join('');
